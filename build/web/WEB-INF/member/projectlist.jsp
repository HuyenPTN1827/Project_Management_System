                            ...
                            <div class="card">
                                <div class="card-header">
                                    <div class="d-flex justify-content-between align-items-center" style="margin: 10px;">
                                        <form action="project-management" method="post" class="d-flex align-items-center" style="gap: 15px;">
                                        ...
                                        <a class="btn btn-primary" href="<%=request.getContextPath()%>/add-project">Create new</a>
                                    </div>
                                </div>
                            </div>
                            <div class="card">
                                        ...
                                        <tbody>
                                            <c:forEach items="${requestScope.listProjects}" var="project">
                                                <tr style="text-align: center">
                                                    ...
                                                    <td>
                                                        <a href="<%=request.getContextPath()%>/edit-project?id=${project.id}" 
                                                           class="btn btn-link text-primary">Details</a>

                                                        <c:if test="${project.status eq 'false'}">
                                                            <a href="<%=request.getContextPath()%>/change-status-project?id=${project.id}&status=${project.status}"
                                                               class="btn btn-link text-success"
                                                               onclick="return confirm('Are you sure you want to activate this project?');">Activate</a>
                                                        </c:if>

                                                        <c:if test="${project.status eq 'true'}">
                                                            <a href="<%=request.getContextPath()%>/change-status-project?id=${project.id}&status=${project.status}"
                                                               class="btn btn-link text-danger"
                                                               onclick="return confirm('Are you sure you want to deactivate this project?');">Deactivate</a>
                                                        </c:if>
                                                    </td>
                                                </tr>
                                            </c:forEach>
        ...
        document.addEventListener("DOMContentLoaded", function (event) {
            setTimeout(function () {
                if (localStorage.getItem('popState') !== 'shown') {
                    window.notyf.open({
                        type: "success",
                        message: "Welcome back to Project Management System!"
                    });
